...
select distinct sex from student_tmp;
...
--错误
select * from student_tmp cluster by sex order by age desc;
select * from student_tmp cluster by sex sort by age desc;

--正确
select * from student_tmp distribute by sex sort by age desc;

--下面两个语句执行结果一样
select * from student_tmp distribute by num sort by num;
select * from student_tmp cluster by num;
...
select sno,sname from student_local
UNION ALL
select sno,sname from student_hdfs limit 2;
...
--select语句中的CTE
with q1 as (select sno,sname,sage from student_tmp where sno = 95002)
select *
from q1;
